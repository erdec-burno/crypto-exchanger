...
export const App = ({ url = '/', router = 'browser' }: AppProps) => {
  ...
  const mainNavigationItems: NavigationMenuItem[] = [
    { to: '/exchange', label: t('navigation.exchange', { ns: 'web' }) },
    { to: '/support', label: t('navigation.support', { ns: 'web' }) },
    { to: '/terms', label: t('navigation.terms', { ns: 'web' }) },
  ];
  const content = (
      ...
      <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#cffafe,transparent_35%),linear-gradient(135deg,#f8fafc,#e2e8f0)] text-slate-950">
        <header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <AdminLoginLink href={runtimeConfig.adminAppUrl}>
              {t('navigation.login', { ns: 'web' })}
            </AdminLoginLink>
          </div>
        </header>
